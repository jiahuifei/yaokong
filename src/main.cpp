...
{ 
	...
	while (true)
	{
		...
		ship_kvs.light = keys.kvs.switch_L1;
		ship_kvs.LX = keys.kvs.LX;
		ship_kvs.LY = keys.kvs.LY;
		ship_kvs.RX = keys.kvs.RX;
		ship_kvs.RY = keys.kvs.RY;
		ship_kvs.DI = map(keys.kvs.L_knob, -100, 100, 0, 100);

		// 一次最多可以传输32个字节。
		nrf.radio.write(&ship_kvs, sizeof(ship_kvs));
		delay(20);
		// 显示数据
		screen.spr.fillSprite(TFT_BLACK);
		screen.spr.drawString("LX: " + String(ship_kvs.LX), 30, 20);
		screen.spr.drawString("LY: " + String(ship_kvs.LY), 30, 60);
		screen.spr.drawString("RX: " + String(ship_kvs.RX), 30, 100);
		screen.spr.drawString("RY: " + String(ship_kvs.RY), 30, 140);
		screen.spr.drawString("DI: " + String(ship_kvs.DI), 30, 180);
		screen.spr.drawString("light: " + String(ship_kvs.light), 30, 220);
		screen.spr.drawString("QUIT: X键", 30, 400);
...
void ship()
{
	...
	struct SHIP_KVS
	{
		...
		bool L_fine_tuning = false;
		bool R_fine_tuning = false;
	...
	while (true)
	{
		...
		ship_kvs.RX = keys.kvs.RX;
		ship_kvs.light = keys.kvs.switch_L1;
		ship_kvs.L_fine_tuning = keys.kvs.left;
		ship_kvs.R_fine_tuning = keys.kvs.right;
		ship_kvs.power_limit = keys.kvs.switch_R1;
		ship_kvs.power = map(keys.kvs.L_knob, -100, 100, 0, 100);
		ship_kvs.turn = map(keys.kvs.R_knob, -100, 100, 0, 100);
		...
		screen.spr.drawString("LY: " + String(ship_kvs.LY), 30, 60);
		screen.spr.drawString("RX: " + String(ship_kvs.RX), 30, 100);
		screen.spr.drawString("light: " + String(ship_kvs.light), 30, 180);
		screen.spr.drawString("p_limit: " + String(ship_kvs.power_limit), 30, 140);
		screen.spr.drawString("power: " + String(ship_kvs.power), 30, 220);
		screen.spr.drawString("turn: " + String(ship_kvs.turn), 30, 260);
		screen.spr.drawString("QUIT: X键", 30, 400);
